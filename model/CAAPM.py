...
# -----------------------------------------------------------------------
        ...
        self.select_attn = Topm_CrossAttention_Restormer(ch_dim, num_heads, bias=False, embed_dim=lin_ch) # Topm_CrossAttention_Restormer
    ...
    def forward(self, img_featur, text_code):
        ...
        att = self.select_attn(self.norm1(q), self.norm2(img_feature2), img_embedding)
...
#     def forward(self, x_q, x_kv, embedding_z):
#         b,c,h,w = x_q.shape
...
#         # 使用 embedding 來預測 r 值：每張圖產生 H 個 r 值
#         r = self.r_predictor(embedding_z)  # [B, H]
#         r = r.clamp(0.1, 1.0)  # 避免 r 為 0
#         k_top_list = (r * N).long()  # 每張圖每個 head 的 top-k 數量
...
class Topm_CrossAttention_Restormer(nn.Module):
    def __init__(self, dim, num_heads, bias, embed_dim):
        ...
        # r 預測器：根據每張圖的 embedding 預測每個 head 的 r 值（介於 0~1）
        self.r_predictor = nn.Sequential(
            nn.Linear(embed_dim, num_heads),
            nn.Sigmoid()
        )
    ...
    def forward(self, x_q, x_kv, embedding_z):
        ...
        # --- Step 1: 根據 embedding 預測每個 head 的 r 值 ---
        r = self.r_predictor(embedding_z)        # [B, H]
        r = r.clamp(min=0.1, max=1.0)
        k_top = (r * N).long().clamp(min=1, max=N)  # [B, H]

        # --- Step 2: 動態 Top-k 遮罩 ---
        topk_values, topk_indices = torch.topk(attn, k=N, dim=-1)  # 每列排序
        rank_idx = torch.arange(N, device=attn.device).view(1, 1, 1, N)  # [1, 1, 1, N]
        k_top_exp = k_top.view(B, self.num_heads, 1, 1)                   # [B, H, 1, 1]
        mask = (rank_idx < k_top_exp).float()                            # [B, H, 1, N]

        scatter_mask = torch.zeros_like(attn)
        scatter_mask = scatter_mask.scatter(-1, topk_indices, mask)     # [B, H, N, N]
        attn = attn.masked_fill(scatter_mask == 0, float('-inf'))
        attn = torch.softmax(attn, dim=-1)

        # Attention output
        out = attn @ v                                                  # [B, H, C//H, N]
        out = out * self.attn4
        out = rearrange(out, 'b head c (h w) -> b (head c) h w', head=self.num_heads, h=H, w=W)
        out = self.project_out(out)
        return out
