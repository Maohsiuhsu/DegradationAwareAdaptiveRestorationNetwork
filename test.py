...
class Trainer():
    def __init__(self, model, conf, device):
        ...
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min', factor=0.9, patience=5, cooldown=2, verbose=True)
    ...
    def inference(self, model_weight_path, inference_epoch_result_root):
        ...
        for inference_dataset in self.inference_datasets:
            ...
            for idx in range(data_len):
                ...
                all_img_result, _, _ = self.model(inference_img, inference_img)
                test_img = process_img(all_img_result)
                
                cv2.imwrite(os.path.join(inference_epoch_result_root, inference_dataset.dataset_version, input_img_fname), test_img)
