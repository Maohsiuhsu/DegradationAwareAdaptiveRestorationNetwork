...
from preprocess.load_data import origin_dataset_csv, inference_dataset_csv
...
class Trainer():
    def __init__(self, model, conf, device):
        ...
        ## Load Datasets
        origin_train_dataset = origin_dataset_csv(dataset_dir=conf["Training"]["dataset_path"], dataset_version="train", device=device, testing_mode=conf["Training"]["debug"], conf=conf)
        origin_val_dataset = origin_dataset_csv(dataset_dir=conf["Training"]["dataset_path"], dataset_version="val", device=device, testing_mode=conf["Training"]["debug"], conf=conf)
        self.train_loader = DataLoader(dataset=origin_train_dataset, batch_size=conf["Model"]["batch_size"], shuffle=True, pin_memory=True, num_workers=num_process, worker_init_fn=seed_worker, generator=g)
        self.val_loader = DataLoader(dataset=origin_val_dataset, batch_size=conf["Model"]["batch_size"], shuffle=False, pin_memory=True, num_workers=num_process, worker_init_fn=seed_worker, generator=g)
        ...
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=conf["Model"]["lr"])
        self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min', factor=0.9, patience=5, cooldown=2,)
        self.loss_logger = loss_csv_logger(acc_csv_path=conf["Training"]["loss_csv_path"], loss_types=["reconstruction", "recognition"])
        self.ckp_saver = model_checkpoint_saver(model=self.model, ckp_path=conf["Training"]["save_weight_root"], save_all_epoch_mode=False)
        ...
        self.early_stop_scheduler = early_stopper(patience=20, star_from_epoch=10)

    def train(self):
        print('='*20)
        print("Model Training: ")
        loss_metrics = {'train_loss': [], "train_reconstruction_loss": [], "train_recognition_loss": []
                        , 'valid_loss': [], "val_reconstruction_loss": [], "val_recognition_loss": []}
        for epoch in range(self.max_epoch):
            print("-"*20)
            print("Time: {now_time}, Epoch: {epoch}".format(now_time=datetime.now().strftime("%d/%m/%Y %H:%M:%S"), epoch=epoch))
            train_loss, train_reconstruction_loss, train_recognition_loss = self.train_loop(epoch=epoch)
            valid_loss, valid_reconstruction_loss, valid_recognition_loss = self.val_loop(epoch=epoch)
            
            self.scheduler.step(valid_loss)
            print('lr:', self._get_lr())
            print('train loss:', train_loss)
            print('train reconstruction_loss:', train_reconstruction_loss)
            print('train recognition_loss:', train_recognition_loss)
            print('valid loss:', valid_loss)
            print('valid reconstruction_loss:', valid_reconstruction_loss)
            print('valid recognition_loss:', valid_recognition_loss)
            ...
            losses_dict = {'train_loss': train_loss, "train_reconstruction_loss": train_reconstruction_loss, "train_recognition_loss": train_recognition_loss
                        , 'valid_loss': valid_loss, "val_reconstruction_loss": valid_reconstruction_loss, "val_recognition_loss": valid_recognition_loss}
    ...
    def train_loop(self, epoch):
        ...
        for input_imgs, gt_imgs, _ in tqdm(iter(self.train_loader), desc="Train"):
        ...
        return avg_loss, avg_reconstruction_loss, avg_recognition_loss
    ...
    def val_loop(self, epoch):
        ...
        for input_imgs, gt_imgs, _ in tqdm(iter(self.val_loader), desc="Valid"):
    ...
    if not (conf['Model']['type']=='airdunet' or conf['Model']['type']=='residual_denseunet_v1_air'):
        ...
        with open(file_path, 'w') as f:
            with redirect_stdout(f):
                summary(model, input_size = [(1, 88, 88)], device=device)
